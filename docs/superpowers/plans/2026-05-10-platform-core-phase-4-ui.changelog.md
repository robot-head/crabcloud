...
- **`ui_router()` reference removed for `dx`-free deployment**: Spec §8.1 mentions a `ui_router()` helper; the actual code uses `.fallback(routes::ui::handler)` directly in `crabcloud-http::router::build_router`. Functionally equivalent. Consider adding a thin `ui_router()` shim for spec traceability.
- **`.contains()`-based HTML assertions in integration tests** (`crates/crabcloud-ui/tests/ssr_routes.rs`) could yield false positives on markup drift. Tighten via DOM-aware assertions in Phase 5.
