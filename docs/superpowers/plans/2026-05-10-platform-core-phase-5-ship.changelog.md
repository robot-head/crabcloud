...
- **`AppConfigService` instrumentation**: cache `set` / `del` failures now emit `tracing::warn!` with structured fields (error, appid, key). `APPCONFIG_CACHE_TTL` lifted to a module constant.
- **`version` subcommand expansion**: prints crate version, git SHA (via a `std::process::Command` build script — `vergen-gix` was planned but pulls in a transitive that requires Rust 1.88, exceeding our 1.85 MSRV), supported dialects, sub-project marker. Closes spec §10.2 / §10.5 acceptance.
